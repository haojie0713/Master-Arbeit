...
def preprocessPoint_training(points, joints, scales):
    points = points * 0.01
    joints = joints * 10.0
    ...
    # rand scaling
    globalRandScale = np.float32(np.maximum(np.minimum(np.random.normal(1.0, 0.3), 1.25), 0.7)) # 0.75 -> 0.6 0.0075 -> 0.5
...
def preprocessPoint_validation(points, joints, scales):
...
def augmentation(points, joints):
    ...
    scale = np.float32(np.maximum(np.minimum(np.random.normal(0.9, 0.28), 1.0), 0.6)) / object_max_dis
    ...
    d_xyz = np.random.normal(joints_center, 0.25) - np.array([0, 0, 0.3])
